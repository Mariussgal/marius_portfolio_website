      ...
      <div className="flex items-center gap-3 px-8 md:px-16 pt-6 pb-4">
        {tabs.map((tab) => (
          <button key={tab.value} onClick={() => setActiveTab(tab.value)}
            className={`px-5 py-2 rounded-[14px] text-[13px] font-medium transition-all duration-300 ${
              activeTab === tab.value ? 'bg-[#4B65FF] text-white shadow-[0_4px_12px_rgba(75,101,255,0.3)]' : 'bg-[#EEF1FF] text-[#4B65FF] hover:bg-[#E4E9FF]'
            }`}>
            {tab.label}
          </button>
        ))}
